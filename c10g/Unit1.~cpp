...
#define  MAP_SIZE_X 2700            // Размер карты высот Terrain.RAW
...
/*
   if((MAP_SIZE_X+MAP_SIZE_Y)/(2*STEP_GRID)>5)
   {
      int i=0;
      for(int yy=0; yy<7; yy++)
         for(int xx=0; xx<7; xx++)
         {
            i = (pHeightMap[(x+xx-3) * MAP_SIZE_Y + y+yy-3]);
            if(i > 10 || i < 9)temp = temp + int((float)i/2); //(1+fabs(yy-3 + xx-3));
            i++;
         }
...
int DrawGLScene(GLvoid)
{
   ...
   glBegin( GL_LINES );		// Рендеринг линий
  	...
  	for ( X = 0; X < (STEP_GRID); X = X + STEP_SIZE)
		for ( Y = 0; Y < (STEP_GRID); Y = Y + STEP_SIZE)
		{
         ...
         points[X][Y][0] = double((X * sin((float)Y/(STEP_GRID))/5.0f)-wMov);// double((r * (sin(tetta) * cos(fi))/5.0f)-wMov);
			points[X][Y][1] = double((Y * cos((float)X/(STEP_GRID))/5.0f)-hMov);//double((r * (sin(tetta) * sin(fi))/5.0f)-hMov);
         points[X][Y][2] = double(z * ScaleHeight * 10 * cos(M_PI_2*y/STEP_GRID) + 10*cos(M_PI_2*y/STEP_GRID));

           /*
         points[X][Y][0] = double((X/5.0f) - wMov);
			points[X][Y][1] = double((Y/5.0f) - hMov);
         points[X][Y][2] = double(z * cos(tetta) * ScaleHeight);
           */
      }
    	glEnd();
   ...
   //if(STEP_GRID<=(MAP_SIZE_Y-1) && STEP_GRID<=(MAP_SIZE_X-1))STEP_GRID++;
	return true;
}
//----------------------------------------------------------------------------
